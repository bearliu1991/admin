<template>
  <div class="ie9-upload">
    <div id="webuploader-hide-block" ref="webuploaderhideblock">
        <div id="webuploader-pickfiles" class="cursor"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    swfPath: {
      type: String,
      default: function() {
        return './static/webuploader/Uploader.swf'
      }
    }
  },
  data() {
      ...
      WebUploader.Uploader.register(
        ...
        },
        {
          beforeSendFile: function(file) {
            ...
            mi.uploadService.callGet(
              'filecheck?md5={md5}&size={size}&ext={ext}',
              { md5: md5, size: size, ext: ext },
              function(data) {
                if (data.exists) {
                  file.path = data.path
          ...
          beforeSend: function(block) {
            ...
            mi.uploadService.callGet(
              'chunkcheck?md5={md5}&chunk={chunk}&size={size}&ext={ext}',
              { md5: md5, chunk: block.chunk, size: block.file.size, ext: ext },
              function(data) {
                if (data.exists) {
                  task.reject()
                } else {
