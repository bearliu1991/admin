    ...
    methods: {
      ...
      getRecords() {
        ...
        this.ajaxRecordsList(param).then((res) => {
          ...
          if (res.code === 1) {
            ...
            this.tableList = this.dealTableList(res.data.records)
      ...
      abondOrder(fn) {
        let param = {
          ...
          orderStatus: this.curOrderInfo.status,
          corpId: this.getCookie('currentCorp').applyId
        }
        this.$get(this.orderPath.cancelOrder, param).then((res) => {
          this.wipeModal()
          if (res.code === 1) {
        ...
        let a = list.map((item) => {
          return {
            firstObj: {
              ...
              serviceName: item.corpName, 
              orderId: item.orderId,
              payOrderNo: item.payOrderNo
            },
            timeLimit: item.validDay,
            priceObj: {
              price: item.goodsPrice, originPrice: item.goodsDiscountsPrice
            },
            quantity: item.goodsNum,
            userObj: {
              picUrl: item.picUrl, mobile: item.orderMobile, nickName: item.orderNickname
            },
            payObj: {
              moneyPaid: item.orderPayprice, payWay: item.payType
            },
            status: item.status
          }
        })
      ...
      handle(key, param) {
        switch (key) {
          ...
          case 'cancelOrder': 
            ...
            this.okText = '确定' 
            this.cancelText = '再想想'
            this.tips = '订单尚未付款, 确定要取消吗'         
            break
          // 立即支付
          case 'orderPayNow':
            this.drawer = false
            this.setCookie('orderId', param.orderId)
            this.setCookie('corpId', this.corpInfo.corpId)
            this.setCookie('corpName', this.getCookie('currentCorp').corpName)
          ...
          case 'orderClose': 
            ...
            this.curOrderInfo = param
            this.tips = '您的订单未及时支付, 已自动关闭, 您可重新订购或查看订单详情。'  
            break
          case 'getRecords': 
            this.getRecords()
            break
