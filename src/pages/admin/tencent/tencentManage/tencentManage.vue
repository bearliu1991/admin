<template>
  <div id="tencentMange">
      ...
      <div class="tencentManage-wrapper" v-show="!noData">
        <div class="table-wrapper">
          ...
          <div class="table-list">
            <Table :loading="loadingImg" :row-class-name="rowClassName" :columns="columns7" :data="tencentListData"></Table>
          </div>
        </div>
      </div>
    </data-loading>
    ...
    <Modal v-model="modalSync" :styles="{top: '33%', marginTop: '-8%'}">
      ...
      <div class="modalSync">
        <p>温馨提示</p>
        <div>1、为了您更好的使用销大师系统，一键同步将从微信平台同步以下数据信息至销大师系统，以确保销大师系统与微信系统一致。</div>
        ...
        <div class="radios">
          ...
          <div class="single-radio">
            <CheckboxGroup v-model="checkAllGroup" @on-change="checkAllGroupChange">
              ...
              <Checkbox label="MEDIA_IMG"><span class="check-right">素材数据</span></Checkbox>
      ...
      <div slot="footer">
        ...
        <Button type="ghost" @click.stop="cancelSync">取消</Button>
      </div>
    </Modal>
...
const accountArr = ['MEDIA_VOICE', 'MEDIA_VIDEO', 'MEDIA_NEWS']
export default {
  name: 'tencentMange',
  data() {
    return {
  ...
  methods: {
    ...
    updateCssWxPublicAccountStatusToDelete(params) {
      updateCssWxPublicAccountStatusToDelete(params).then(data => {
        switch (data.code) {
          ...
          //   break
          // case 6005:
          //   this.$Message.error('当前已为删除状态')
          //   break
          // case 6005:
          //   this.$Message.error('您的企业在禁用或启用状态下无法直接删除')
          //   break
          default:
    ...
    // 公众号信息一键同步
    updateCssWxPublicAccountDataToDb() {
      let updatePublicAccountData = JSON.parse(JSON.stringify(this.checkAllGroup))
      if (updatePublicAccountData.indexOf('MEDIA_IMG') >= 0) {
        updatePublicAccountData = updatePublicAccountData.concat(accountArr)
      }
      let obj = {
