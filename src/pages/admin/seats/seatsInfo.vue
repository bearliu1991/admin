            ...
            <TabPane label="坐席权限" name="2">
              ...
              <scroll-bar>
                <div class="info-scroll" v-show="seatsInfo.seatPublicNumDetailDtos">
                  ...
                  <!-- <div class="limit-name" v-for="(chid, indx) in corpPackRoleList" :key="indx">
                    <div class="limit-title">{{chid.name}}</div>
                    <div class="limit-account-list">
                      <div class="limit-account-list-item" v-for="(item, key, index) in chid.obj" :key="index">
                        <FoldPanel :isFold="true">
                          <div slot="title">
                            {{item.name}}
                          </div>
                          <div slot="container">
                            <div class="container-item"  v-for="(n, idx) in item.data" :key="idx">{{n.count}}</div>
                          </div>
                        </FoldPanel>
                      </div>
                    </div>
                      ...
                      <div class="menu-con" :class="{'un-active': item.status!=1}">
                        ...
                        <div class="menu-con-item">
                          ...
                          <span class="item-info">
                            <UploadImg :isUpload="false" v-if="item.conditionalHeadImg || item.headImg" :imgSrc="item.conditionalHeadImg || item.headImg"></UploadImg>
                          </span>
                        </div>
                      </div>
                    </div>
